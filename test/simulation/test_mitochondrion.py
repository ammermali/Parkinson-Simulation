...
from testhelpers import TestAggregateAgent, TestAgent, TestRng, import_any
...
def make_mitochondrion(neuron: Neuron, config: Optional[MitochondrionConfig] = None) -> Mitochondrion:
    mitochondrion = Mitochondrion(
        ...
        rank=0,
        type_id=30,
        config=config or MitochondrionConfig(),
        owner_neuron=neuron,
    )
...
class TestMitochondrion:
    def test_see_reads_neuron_context_and_target_assignment(self):
        ...
        neuron.add_agent(TestAggregateAgent(aggregate_weight=1.0, uid=2), DiscretePoint(1, 1))
    ...
    def test_healthy_mitochondrion_signals_stress_in_high_damage_context(self):
        ...
        assert neuron.internal_effects.oxidative_stress_added == pytest.approx(
            mitochondrion.cfg.stress_release_rate
        )
    ...
    def test_damaged_mitochondrion_registers_for_lysosomal_degradation(self):
        ...
        assert neuron.internal_effects.debris_added == pytest.approx(
            mitochondrion.cfg.debris_release_rate
        )
