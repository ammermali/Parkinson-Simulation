...
from src.analysis.g0_lexer import build_g0_graph
...
class TestG0Lexer:
    def test_builds_state_transition_graph_and_continuity_edges(self, tmp_path):
        ...
        assert graph.nodes["AlphaSynuclein_a:1.Misfolded@1.2"]["display_id"] == "AlphaSynuclein_a:1_Misfolded@1.2"
    ...
    def test_collapses_action_node_into_field_effect_edge(self, tmp_path):
        ...
        assert "Neuron_n:1.release_dopamine@1.3" not in graph
        assert graph.has_edge("Neuron_n:1.Healthy@1.2", "SN.dopamine_output_buffer@1.4")
        edge_data = next(iter(graph.get_edge_data("Neuron_n:1.Healthy@1.2", "SN.dopamine_output_buffer@1.4").values()))
    ...
    def test_keeps_lysosome_targeting_and_alpha_aggregation_edges(self, tmp_path):
        ...
        assert "agent_relation" in edge_kinds
        assert "aggregation" in edge_kinds
        assert graph.nodes["n:1::Aggregate_1.Oligomer@4.4"]["semantic_kind"] == "agent_state"
