...
`AlphaAggregate` represents a set of aggregated alpha-synuclein proteins. It is the unit used for grid representation, lysosomal degradation, local aggregate density and Lewy body maturation.

## Responsibility

Single misfolded `AlphaSynuclein` proteins can declare the intention to oligomerize, but the actual decision is collective because it requires more proteins in the same cell or the presence of a recruiting aggregate.
...
- `owner_neuron`: neuron to which it belongs, or `None` if it is extracellular.
...
2. Collects `AlphaSynuclein` proteins that are free, misfolded and have `wants_oligomerization = True`
3. Collects aggregates in the same cell
4. If a Lewy body exists, chooses the largest one as a seed, then merges the other recruiting aggregates and absorbs candidate proteins.
5. If there are recruiting oligomers, chooses the largest one, merges the others and absorbs the candidate proteins.
...
- registered aggregate but absent from the grid of the owner neuron;
- Lewy body with members not `LewyBody`.

## Degradation

Aggregates are exposed to the lysosomal buffer within the neuron. If a `Lysosome` successfully degrades an aggregate:
...
- With `AlphaSynuclein`: represents members that are no longer active in a grid.
- With `Neuron`: lives in the intracellular grid, contributes to alpha_load and gets targeted.
- With `Lysosome`: can overwhelm the lysosome or get degraded by it.
- With other aggregates: can be merged by the registry.
