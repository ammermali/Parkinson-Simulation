...
from src.analysis.schemes.contraction_utils import iter_edges, make_digraph, merge_summary_edge, summarize_nodes, summarize_superedges, summarize_supernodes, time_group_key
...
class TimeContractionScheme(ContractionScheme):
    ...
    def contract_networkx(self, g0_graph):
        ...
        for key, nodes in sorted(buckets.items()):
            g1.add_node(key, **summarize_nodes(nodes, key=key, contraction="time", level="G1"))
    ...
    def contraction_function(self, dec_graph):
        ...
        return CompTable(ComponentSet(self._get_component_set_id(),nodes,**self._component_set_attr(nodes)) for nodes in grouped.values())
